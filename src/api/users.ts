    ...
    const newUser = await db.transaction(async (tx) => {
      const u = await tx
        ...
        .values({
          ...
          supabaseUid: crypto.randomUUID(), // Mock for now until Supabase Admin API is used
        })
        .returning({
          id: users.id,
          email: users.email,
