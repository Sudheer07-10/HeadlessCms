...
apiMedia.use('*', async (c, next) => {
  ...
  try {
    ...
    const cookieTenant = getCookie(c, 'cms_active_tenant')
    const headerTenant = c.req.header('mycms-tenant-id') || c.req.header('X-Tenant-ID')
...
apiMedia.post('/upload', async (c) => {
  ...
  try {
    ...
    const uploadPreset = process.env.CLOUDINARY_UPLOAD_PRESET || 'my-cms'
    ...
    if (tenantId) {
      ...
      if (tenantData.length > 0) {
        ...
        if (storageService === 'SUPABASE') {
          await createSupabaseFolder(folderPath)
        } else {
    ...
    if (storageService === 'SUPABASE') {
      result = await uploadBufferToSupabase(buffer, file.name, folderPath, file.type)
    } else {
...
apiMedia.delete('/:id', async (c) => {
  ...
  try {
    ...
    if (mediaItem.publicId) {
      if (storageService === 'SUPABASE') {
        await deleteFromSupabase(mediaItem.publicId)
      } else {
