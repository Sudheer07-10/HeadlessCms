...
console.log('🔥 My CMS: Hono Initializing on Vercel Node Runtime')
...
app.use('*', async (c, next) => {
  ...
  if (apiKey) {
    ...
    authType = 'session'
    try {
      // Use Supabase Admin SDK to verify the token (supports both HS256 and ES256)
      ...
      if (error || !supabaseUser) throw error || new Error('Invalid token')
...
app.use('*', async (c, next) => {
  if (!c.req.path.startsWith('/api/')) {
    ...
    c.set('inertiaSharedProps' as any, {
      ...
      appDomain: process.env.APP_DOMAIN || 'my-cms.com',
      features,
      canCreateWorkspace,
...
app.get('/login', async (c) => {
  ...
  return c.get('inertia')('Index', { title: 'My CMS' })
})
...
// Visual Editor Route
app.get('/visual-editor', requireAuth, async (c) => {
  const userData = c.get('user')
  return c.get('inertia')('VisualEditor', {
    user: userData,
    title: 'Visual Editor',
  })
})
...
api.use('*', async (c, next) => {
  ...
  if (
    ...
    path === '/api/test' ||
    c.req.header('X-My-CMS-Test') === 'true' ||
        ...
        if (status === 'active' || status === 'trialing') {
          ...
          if (user && user.planTier !== 'PRO') {
            try {
              await sendEmail({
                to: user.email,
                subject: 'Thank you for subscribing to My CMS Pro!',
                html: `
                  <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 8px; color: #555; line-height: 1.6;">
                    <h2 style="color: #87787a; border-bottom: 2px solid #514849; padding-bottom: 10px; font-size: 24px; font-weight: bold; margin-bottom: 24px;">Welcome to My CMS Pro!</h2>
                    <p style="font-size: 16px; color: #555;">Hi ${user.name || 'there'},</p>
                    <p style="font-size: 16px; color: #555;">Thank you for subscribing to our <strong>My CMS Pro</strong> plan! We are thrilled to have you on board.</p>
                    ...
                    <p style="font-size: 15px; color: #555;">If you have any questions, encounter any issues, or need help setting up your endpoints, feel free to reply directly to this email or reach out to us at <a href="mailto:support@my-cms.com" style="color: #514849; text-decoration: underline;">support@my-cms.com</a>.</p>
                    <hr style="border: 0; border-top: 1px solid #eee; margin: 20px 0;" />
                    <p style="font-size: 14px; color: #666;">Cheers,<br><strong style="color: #514849;">The My CMS Team</strong></p>
                  </div>
                `,
              })
              console.log(
...
api.post('/tenants/switch', async (c) => {
  ...
  if (tenantId === null) {
    ...
    const cookieOpts = getCookieOptions(c, 0)
    deleteCookie(c, 'cms_active_tenant', {
      path: cookieOpts.path,
      domain: cookieOpts.domain,
    })
    return c.json({ success: true })
...
api.post('/test-email', async (c) => {
  ...
  try {
    ...
    const result = await sendEmail({
      to,
      subject: 'My CMS: Transactional Email Test',
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; rounded: 8px;">
          ...
          <p style="font-size: 16px; line-height: 1.6; color: #555;">
            Hello! This is a test email from your <strong>My CMS</strong> instance.
          </p>
          ...
          const htmlContent = `
            <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 8px;">
              ...
              <footer style="margin-top: 30px; padding-top: 15px; border-top: 1px solid #eee; font-size: 12px; color: #666; text-align: center;">
                <p style="margin: 5px 0;">This email is sent automatically from My CMS.</p>
