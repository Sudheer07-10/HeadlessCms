...
const SPONSORS = [
  {
    ...
    href: 'https://shotage.studio?ref=my-cms.com',
  ...
  },
  {
    ...
    href: 'https://heylookat.me?ref=my-cms.com',
...
export default function Layout({ user, title, children }: LayoutProps) {
  ...
  React.useEffect(() => {
    if (sessionStorage.getItem('mycms-sponsored-dismissed')) return

    const savedSponsorId = sessionStorage.getItem('mycms-sponsored-choice')
    ...
    if (!savedSponsor) {
      sessionStorage.setItem('mycms-sponsored-choice', selectedSponsor.id)
    }
    setSponsor(selectedSponsor)
  }, [])

  const dismissSponsor = () => {
    sessionStorage.setItem('mycms-sponsored-dismissed', 'true')
    setSponsor(null)
  }
  ...
  const handleTenantSwitch = (tenantId: number | null) => {
    ...
    fetch('/api/tenants/switch', {
      ...
      if (res.ok) {
        ...
        const baseDomain = appDomain || 'my-cms.com'
  ...
  return (
    <TooltipProvider>
      <Head title={title ? `${title} | My CMS` : 'My CMS'}>
      ...
      <div className='min-h-screen flex bg-background text-foreground'>
        ...
        >
          ...
          >
            <a
              href='https://github.com/bayukurniawan30/my-cms'
              target='_blank'
              rel='noopener noreferrer'
              title={`My CMS ${getAppVersion()} on GitHub`}
              className={cn(
            ...
            >
              ...
              >
                My CMS <span className='font-mono'>{getAppVersion()}</span>
              </span>
            </a>
            <Tooltip delayDuration={0}>
              <TooltipTrigger asChild>
                {isSelfHosted ? (
                  ...
                  <a
                    href={`mailto:support@my-cms.com?subject=My CMS%20CMS%20Support&body=${encodeURIComponent(
                      `User: ${user?.email || ''}\nActive Workspace: ${
                        activeTenant ? activeTenant.name : 'System Global'
                      }\n\n`
                    )}`}
                    title='support@my-cms.com'
                    className={cn(
                      'flex items-center gap-3 px-4 py-2 rounded-md font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-colors text-sm',
                      !isSidebarOpen && 'lg:justify-center lg:px-0'
                    )}
