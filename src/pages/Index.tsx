  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (step === 1) {
        ...
        if (authData.session) {
          document.cookie = `cms_token=${authData.session.access_token}; path=/; max-age=${authData.session.expires_in}`
        }
        ...
        if (verify.error) {
           setError('email', verify.error.message)
           setIsSubmitting(false)
           return
        }
        ...
        if (sessionData.session) {
          document.cookie = `cms_token=${sessionData.session.access_token}; path=/; max-age=${sessionData.session.expires_in}`
        }
        window.location.href = '/dashboard'
      }
    } catch (err: any) {
      setError('email', err.message || 'A network error occurred. Please try again.')
      setIsSubmitting(false)
    }
  }
        ...
        <div className='text-center mb-4 space-y-4'>
          ...
          <div className='flex items-center justify-center space-x-2 mb-6'>
            <h1 className='text-4xl font-black tracking-tighter text-foreground'>
              MY CMS <span className='text-primary'>CMS</span>
            </h1>
          </div>
