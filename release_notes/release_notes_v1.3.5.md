# My CMS Release Notes - v1.3.5

We are excited to release **My CMS v1.3.5**! This release introduces dynamic custom domain support for multi-tenant subdomain routing, enabling self-hosted installations to route subdomains, set session cookies, and switch workspaces under their own customized domain.
...
- **`APP_DOMAIN` Configuration**: Added support for an `APP_DOMAIN` environment variable (falling back to `my-cms.com`).
