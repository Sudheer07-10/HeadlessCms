...
export const REPOSITORY_URL =
  'https://github.com/bayukurniawan30/my-cms.git'
export const DEFAULT_REF = 'v1.4.1'

const HELP = `
Create a self-hosted My CMS project.

Usage:
  npx create-my-cms@latest <project-directory> [options]
  ...
  try {
    ...
    if (options.databaseUrl && !options.adminEmail) {
      options.adminEmail = await input.question(
        'Initial admin email [admin@my-cms.com]: '
      )
      options.adminEmail ||= 'admin@my-cms.com'
    }
...
async function cloneStarter(targetDirectory, ref) {
  const temporaryDirectory = await mkdtemp(`${tmpdir()}/create-my-cms-`)
  try {
    await command('git', [
      'clone',
      '--depth',
...
export async function run(argv = process.argv.slice(2)) {
  ...
  console.log(`\nCreating My CMS in ${targetDirectory}...`)
  ...
  console.log(`\n✓ My CMS is ready in ${basename(targetDirectory)}\n`)
